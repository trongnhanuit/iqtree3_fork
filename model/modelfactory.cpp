...
string::size_type posGenotype(const string& name) {
    ...
    for (pos1 = 0; pos1 != string::npos; pos1++) {
        ...
        if (pos1 == string::npos)
            break;
        if (pos1+5 >= name.length() || !isalpha(name[pos1+5])) {
            break;
        }
    }
    ...
    for (pos2 = 0; pos2 != string::npos; pos2++) {
        ...
        if (pos2 == string::npos)
            break;
        if (pos2+5 >= name.length() ||!isalpha(name[pos2+5]))
            break;
    }
...
ModelFactory::ModelFactory(Params &params, string &model_name, PhyloTree *tree, ModelsBlock *models_block) : CheckpointFactory() {
    ...
    if ((gt_pos == string::npos) &&
        ...
        outError("For genotype data, model string (" + model_str + ") must not contain, e.g., \"+GT\".");
...
void ModelFactory::restorePomoGenotypeModel(const SeqType& seq_type, const string::size_type& p_pos, string& rate_str, string& model_str)
{
    ...
    // jump_size = 2 (+P) for Pomo and jump_size = 5 (+GT1X) for Genotype
    ...
    if (p_pos != string::npos)
    {
      ...
      if (rate_str[p_pos+jump_size] == '{') {
        ...
        if (close_bracket == string::npos)
          outError("No closing bracket in PoMo/Gynotype parameters.");
        else {
