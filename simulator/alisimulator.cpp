    ...
    if (params->partition_file) {
        ...
        if (params->partition_type == BRLEN_SCALE)
        {
            ...
            if (sum > 1 + epsilon || sum < 1 - epsilon)
            {
                ...
                for (size_t i = 0; i < ((PhyloSuperTree*) tree)->size(); i++)
...
void AliSimulator::getOnlyVariantSites(vector<short int> &variant_state_mask, Node *node, Node *dad){
    if (node->isLeaf() && node->name!=ROOT_NAME) {
        ...
        for (size_t i = 0; i < node->sequence->sequence_chunks[0].size(); i++)
            // only get variant sites
...
void AliSimulator::mergeOutputFiles(ostream *&single_output, int thread_id, string output_filepath, std::ios_base::openmode open_mode, bool write_sequences_to_tmp_data)
{
    ...
    if (output_filepath.length() > 0 && !write_sequences_to_tmp_data)
    {
        ...
        if (num_threads != 1)
        {
            ...
            {
                string output = "";
                for (size_t j = 0; j < input_streams.size(); j++)
                {
        ...
        if (store_seq_at_cache)
        {
            ...
            #endif
            {
                if (num_threads != 1)
                {
                    ...
                    for (size_t thread_id = 1; thread_id < num_simulating_threads; thread_id++)
...
{
    for (size_t thread_id = 0; thread_id < num_simulating_threads; thread_id++)
    {
        for (size_t i = cache_start_indexes[thread_id]; i < cache_start_indexes[thread_id] + cache_size_per_thread; i++)
        {
...
{
    for (size_t i = 0; i < seq_str_cache.size(); i++)
    {
        if (seq_str_cache[i].chunk_status == OCCUPIED)
        {
            #ifdef _OPENMP
...
void AliSimulator::convertProMatrixIntoAccumulatedProMatrix(double *probability_maxtrix, int num_rows, int num_columns, bool force_round_1)
{
    ...
    if (force_round_1)
    {
        ...
        {
            for (size_t c = 1; c < num_colmns_minus_one; c++)
    ...
    if (tree->getModel() && tree->getSubstName().find("+ASC") != std::string::npos)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < max_num_states; i++, index += num_sites_per_state)
            {
                ...
                for (size_t j = 0; j < num_sites_per_state; j++)
    ...
    if (tree && tree->aln)
    {
        ...
        if (tree->isSuperTree())
        {
            for (size_t i = 0 ; i < ((PhyloSuperTree*) tree)->size(); i++)
            {
                ...
                for (size_t j = 0; j < seq_names.size(); j++)
                    if (seq_names[j].length() >= max_length_taxa_name)
                        max_length_taxa_name = seq_names[j].length() + 1;
            }
        }
...
void AliSimulator::permuteSelectedSites(vector<FunDi_Item> &fundi_items, Node* node)
{
    ...
    if (std::find(params->alisim_fundi_taxon_set.begin(), params->alisim_fundi_taxon_set.end(), node->name) != params->alisim_fundi_taxon_set.end())
    {
        ...
        for (size_t i = 0; i < fundi_items.size(); i++)
...
void AliSimulator::simulateASequenceFromBranch(ModelSubst *model, int sequence_length, double *trans_matrix, Node *node, NeighborVec::iterator it, int* rstream, default_random_engine& generator, string lengths)
{
    ...
    int segment_start = 0;
    for (size_t i = 0; i < node->sequence->sequence_chunks.size(); i++)
    {
...
void AliSimulator::extractRatesJMatrix(ModelSubst *model)
{
    ...
    for (int mixture = 0; mixture <num_mixture_models; mixture++, starting_index_sub_rates += max_num_states, starting_index_J += max_num_states_square)
    {
        ...
        for (int i = 0; i < max_num_states; i++, i_times_max_num_states += max_num_states)
        {
            ...
            for (size_t j = 0; j < max_num_states; j++)
            {
                if (i == j)
...
int AliSimulator::handleDeletion(int sequence_length, vector<short int> &indel_sequence, double &total_sub_rate, vector<double> &sub_rate_by_site, SIMULATION_METHOD simulation_method, default_random_engine& generator)
{
    ...
    double sub_rate_change = 0;
    for (size_t i = 0; i < length && (position + i) < indel_sequence.size(); i++)
    {
...
void AliSimulator::handleSubs(int segment_start, double &total_sub_rate, vector<double> &sub_rate_by_site, vector<short int> &indel_sequence, int num_mixture_models, std::vector<bool>* const site_locked_vec, int* rstream, default_random_engine& generator)
{
    ...
    for (size_t i = 0; i < indel_sequence.size(); i++)
    {
...
void AliSimulator::handleDNAerr(int segment_start, double error_prop, vector<short int> &sequence_chunk, int* rstream, int model_index)
{
    ...
    {
        for (size_t i = 0; i < sequence_chunk.size(); i++)
...
void AliSimulator::updateNewGenomeIndels(int seq_length)
{
    ...
    for (size_t i = 0; i < insertion->phylo_nodes.size(); i++)
    {
        tips_count++;
...
void AliSimulator::separateSeqIntoChunks(Node* node)
{
    if (node->sequence->sequence_chunks.size() != num_simulating_threads)
    {
        ...
        for (size_t i = 1; i < num_simulating_threads; i++, starting_index += default_segment_length)
        {
            ...
            for (size_t j = 0; j < actual_segment_length; j++)
...
void AliSimulator::mergeChunks(Node* node)
{
    ...
    for (size_t i = 1; i < node->sequence->sequence_chunks.size(); i++)
    {
        for (size_t j = 0; j < node->sequence->sequence_chunks[i].size(); j++)
        {
