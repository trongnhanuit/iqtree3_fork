...
# --analytical-gradients (model/gradientoptimizer.*): same analysis as the plain
# LG+F+G4 run, model parameters optimised with exact gradients. The IQ-TREE 2
# baseline binary does not know the flag; that run fails and is logged as such.
run_timed ${IQTREE_BIN} -s $AA_FASTA -m LG+F+G4 --analytical-gradients --prefix ${OUT_DIR}/turtle_aa.ag -seed $SEED -T 1
