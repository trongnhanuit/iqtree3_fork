...
# pre-defined expected value from expect_memory.txt if a platform column is given.
...
threshold_file="${WD}/expect_memory.txt"
...
# Memory is column 4 of each log (identifier, Command, RealTime, PeakMemory).
...
FILENAME == L2 { if (FNR > 1) { mem2[$1] = $4; cmd2[$1] = $2 } next }
FILENAME == L3 { if (FNR > 1) {
    n++; id[n] = $1; mem3[$1] = $4; cmd3[$1] = $2
} next }
END {
    for (i = 1; i <= n; i++) {
        k = id[i]
        printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", k, haverow[k] ? "Y" : "N",
               threshold[k], fallback[k], mem2[k], mem3[k], cmd2[k], cmd3[k]
    }
    for (k in haverow) if (!(k in mem3)) printf "%s\tORPHAN\t\t\t\t\t\t\n", k
