...
//        ASSERT(!aln->ordered_pattern.empty());
        ...
        size_t nsites = static_cast<size_t>((aln->num_parsimony_sites+NUM_BITS-1)/NUM_BITS);
        dad_branch->partial_pars[static_cast<size_t>(nstates*VCSIZE)*nsites] = 0;
...
//                    int M = value1 + value2;
                    ...
                    if (weight1 < 1.0/4)
                        ...
                        real_state = static_cast<int>((*alnit)->STATE_UNKNOWN);
                    /*
                    if (value1 == 0) 
                        real_state = id2;
                    else if (value1 >= N)
...
//        assert(site == aln->num_parsimony_sites % NUM_BITS);
        ...
        if (site > 0 && site < NUM_BITS) {
            ...
            memset(x, 255, static_cast<size_t>(VCSIZE - max_sites)*sizeof(UINT));
        }
        if (!aln->isSuperAlignment())
            delete partitions;
    } else {
...
//        VectorClass score = 0;
        ...
        dad_branch->partial_pars[static_cast<size_t>(nstates*VCSIZE*nsites)] = score + left->partial_pars[static_cast<size_t>(nstates*VCSIZE*nsites)] + right->partial_pars[static_cast<size_t>(nstates*VCSIZE*nsites)];
    }
}
...
void PhyloTree::computePartialParsimonySankoffSIMD(PhyloNeighbor *dad_branch, PhyloNode *dad){
    ...
    if (node->degree() > 3) {
            ...
            FOR_NEIGHBOR_IT(node, dad, it) if ((*it)->node->name != ROOT_NAME) {
                if ((*it)->node->isLeaf()) {
                    ...
                    for (size_t i = 0; i < VectorClass::size(); i++) {
                        ...
                        const UINT *partial_pars_child_ptr = &tip_partial_pars[aln->ordered_pattern[ptn+i][static_cast<size_t>((*it)->node->id)]*nstates];
                    ...
                    // internal node
                    const VectorClass *partial_pars_child_ptr = (VectorClass*)&(static_cast<PhyloNeighbor*> (*it))->partial_pars[ptn_start_index];
        ...
        for (size_t ptn = 0; ptn < aln->ordered_pattern.size(); ptn+=VectorClass::size()){
            ...
            // load data for tip
            for (size_t i = 0; i < VectorClass::size(); i++) {
                const UINT *left_ptr = &tip_partial_pars[aln->ordered_pattern[ptn+i][static_cast<size_t>(left->node->id)]*nstates];
                const UINT *right_ptr = &tip_partial_pars[aln->ordered_pattern[ptn+i][static_cast<size_t>(right->node->id)]*nstates];
                UINT *tip_buffer_ptr = (UINT*)tip_buffer + i;
...
int PhyloTree::computeParsimonyBranchSankoffSIMD(PhyloNeighbor *dad_branch, PhyloNode *dad, int *branch_subst) {
    ...
    if (dad->isLeaf()) {
        ...
        for (size_t ptn = 0; ptn < aln->ordered_pattern.size(); ptn+=VectorClass::size()){
            size_t ptn_start_index = ptn * nstates;
            for (size_t  i = 0; i < VectorClass::size(); i++) {
                const UINT *node_branch_ptr = &tip_partial_pars[aln->ordered_pattern[ptn+i][static_cast<size_t>(dad->id)]*nstates];
                UINT *tip_buffer_ptr = (UINT*)tip_buffer + i;
                for (size_t j = 0; j < nstates; j++, tip_buffer_ptr += VectorClass::size()) {
    ...
    if (branch_subst)
        *branch_subst = static_cast<int>(horizontal_add(branch_pars));
    //    cout << endl;
    return static_cast<int>(horizontal_add(tree_pars));
}
